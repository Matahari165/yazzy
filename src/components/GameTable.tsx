...
type RollButtonProps = Pick<GameTableProps, "heldCount" | "rollNumber" | "isRolling" | "onRoll"> & {
  className?: string;
};

function RollButton({ heldCount, rollNumber, isRolling, onRoll, className = "" }: RollButtonProps) {
  return (
    <button
      type="button"
      ...
      onClick={onRoll}
    >
      <span>{isRolling ? "ROULE…" : rollNumber === 0 ? "LANCER" : "RELANCER"}</span>
      <small>{rollNumber === 0 ? "les 5 dés" : `${5 - heldCount} dé${5 - heldCount > 1 ? "s" : ""}`}</small>
    </button>
  );
}
    ...
    <>
      <div className="table-surface" data-rolling={isRolling} aria-busy={isRolling}>
        <div className="roll-status" aria-live="polite">
          ...
          <span>{rollNumber === 0 ? "Lance les cinq dés" : "Clique les dés à conserver"}</span>
        </div>
        ...
        <div className="play-actions">
          <RollButton heldCount={heldCount} rollNumber={rollNumber} isRolling={isRolling} onRoll={onRoll} />
      ...
      <section className="mobile-play-dock" aria-label="Commandes de jeu" data-tone={coachTone}>
        ...
        <div className="mobile-dock-main">
          ...
          {selectedCategory ? (
            <button type="button" className="mobile-score-button" disabled={isRolling} onClick={onScore}>
              <span>INSCRIRE {selectedPoints} PTS</span><small>{CATEGORY_BY_ID[selectedCategory].shortLabel}</small>
            </button>
          ) : (
            <p className="mobile-dock-hint">
              {rollNumber === 0 ? "3 lancers pour construire ton coup" : "Touche les dés à garder, puis choisis une case."}
            </p>
          )}
        </div>
      </section>
